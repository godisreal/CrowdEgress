...
(2) The second method is to use command python evac-non-gui.py <filename_CSV> <filename_FDS> to direct start a simulation without using GUI.  Here <filename_CSV> is the .csv file name while <filename_FDS> is the fds. file name.  Clearly <filename_FDS> is optional and users can omit it if only .csv is used in simulation.  Users can also run python evac-non-gui.py --help to show the readme txt in the console.  
